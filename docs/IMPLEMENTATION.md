...
Configured routing Agents classify ordinary language before deterministic expression rules; slash controls bypass classification. Codex classification reuses login with --ephemeral, --ignore-user-config, --ignore-rules and --output-schema, independent empty cwd/state, fixed read-only settings and disabled tool configuration. It rejects tool events and requires valid thread/turn/final/exit/cleanup. HTTP JSON classification remains supported and transmits configured reasoning_effort without temperature. The classifier gets no execution capability. Host code performs directory traversal, native history reading, state selection and final authorization. No production fake backend is introduced. Local routing identity is pinned separately from the selected backend; startup validates pending routed profiles before replacing an instance.
...
The HISTORY_FORMAT incident combined a legacy unscoped rollout with rule-only misclassification of a session progress query. The corrected parser skips recognized no-cwd legacy headers, keeps foreign bodies unread, and displays active sessions without permitting resume. Unknown corrupt candidates are reported as incomplete history; automatic work is blocked rather than silently recreated. Local review also checks the routing Agent process marker and stale classifier lock. The scoped live smoke performs classification and read-only history lookup only; no worker task or Weixin send occurs.
