...
- 移除旧阶段测试日志与重复报告，重新生成当前离线证据并替换本机临时路径/PID。实际消息、模型输出和媒体不进入证据文件。

本次 Git index 共检查 56 个文件，精确比对 22 个真实私有值，未发现匹配；个人路径、私钥/API key/JWT 候选和运行时文件清单检查均无遗留问题。邮箱格式候选是测试中用于拒绝 URL 用户信息的合成值，不是个人联系地址。

扫描和人工检查针对本次发布文件，不是对任意未知秘密格式或运行环境的全面保证。

## Git 历史边界

先前已发布的 macOS 初次失败日志曾含本机绝对路径。本轮从当前文件树移除该旧日志，但没有改写或强推既有 Git 历史，因此旧提交仍可找到该路径。该发现是路径信息暴露，不是发现真实 token 或 API key 泄露。Git 正常作者元数据也不在内容脱敏范围内。
