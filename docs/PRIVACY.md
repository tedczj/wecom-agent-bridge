...
本轮发布扫描检查 Git index 的 67 个文件，个人绝对路径、私钥、API key/JWT 候选和运行时文件清单均无命中。旧报告中的“56 个文件 / 21 个私有值”属于上一轮，不沿用为当前证据；本轮不为发布检查读取真实会话内容或模型回答。

此前 Pi 配置保留在被忽略的 `*.local.json` 中，启动包装器、沙箱 profile、会话及本地 smoke 结果保留在仓库外的私有运行目录。这些私人部署文件本轮未更改或纳入提交。公开验证仅记录版本、结果和受限探针结论，不包含模型回答、认证内容或本机绝对路径。
...
routing_state 中的 active directory、版本化别名、历史预览、列表快照与搜索 continuation 都属于私有状态，不提交仓库。可选解释器密钥从 operator 指定环境变量读取；没有该配置时不会发出解释器请求。配置后会发送当前用户路由文本、有限近期用户上下文和有界项目说明到该 HTTPS endpoint；不传 bot/context token、native 全部历史或 worker 工具输出。根授权和执行 profile 仍由宿主控制，不能由模型返回值扩大。
