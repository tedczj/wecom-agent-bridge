-- Reference additive DDL for design 1.0. NOT a production migration entry point.
-- Requires the existing v3 jobs(task_id) and sessions(session_key) tables.
-- The migration service must backup/drain/validate identities/import bindings itself.
-- Intentionally does not set PRAGMA user_version or modify existing business records.
