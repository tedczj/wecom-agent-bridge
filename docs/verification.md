...
本次在现有 `native_session_catalog` 中补齐业务创建时的原子登记、发现外部原生会话时的角色持久化；管理角色禁止转成业务，完成/可恢复验证仍独立。交互列表、搜索和交接保留 `producerRole`，原生正文工具带回 native ID 和角色。角色指令明确微信中的项目 session/LLM 回复查询指原生业务会话，管理摘要不能代替正文。未新增问法分类器或数据库迁移。

- `npm ci` exit 0，按 lockfile 安装 16 个包；安装审计报告 1 项 high，未进行依赖升级。
- 修复前回归执行 exit 1：33 项中 2 项失败，分别复现业务创建未登记与外部发现未登记。修复后 `npm run check` exit 0，**324/324 OFFLINE PASS**，0 failed/cancelled/skipped。覆盖角色登记、管理角色不可改写、目录身份约束、摘要/交接来源和原生正文来源。完整输出在忽略的 `runtime/session-role-before.txt`、`runtime/session-role-check.txt`。
- 使用在线备份得到的私有数据库副本验证现场记录：截图两条回复在投影和交接中均标为 `route`；所读一页 10 个 term4u 原生候选均登记为 `external`，排除管理角色。覆盖仍为 `partial`，未宣称全量发现。证据 `runtime/session-role-snapshot-fq2viyeb/report.json`；未修改生产库，未调用模型或发送微信。
- 本次验证未包含真实模型语义复测或手机微信验收；离线角色/来源检查不代表模型必然正确读取正文。生产重启结果单独记录在本机 `runtime/session-role-restart.json`。

# 此前验证记录：原生 session 只读访问与引用续接（2026-09-26）
