...
export function log(event: string, fields: { taskId?: string; code?: string; state?: string; bytes?: number; apiRet?: number; apiErrcode?: number } = {}): void {
  ...
  process.stderr.write(JSON.stringify({ event: /^[a-z0-9_.-]+$/.test(event) ? event : 'event',
    ...
    apiRet: Number.isSafeInteger(fields.apiRet) ? fields.apiRet : undefined,
    apiErrcode: Number.isSafeInteger(fields.apiErrcode) ? fields.apiErrcode : undefined }) + '\n');
}
