...
/** Walk every existing component; never chmod a symlink target. */
...
export function acquireLock(root: string): () => void {
  ...
  const file = path.join(root, 'instance.lock'), token = randomUUID();
  try { writeFileSync(file, JSON.stringify({ pid: process.pid, token }), { flag: 'wx', mode: 0o600 }); }
  catch (e) { if ((e as NodeJS.ErrnoException).code === 'EEXIST') throw new BridgeError('INSTANCE_LOCKED'); throw e; }
  return () => { try { if (JSON.parse(readFileSync(file, 'utf8')).token === token) unlinkSync(file); } catch {} };
}
/** Explicit operator action only: a stale lock is not evidence that Agent children stopped. */
