...
export function createBackend(c: Config, media: MediaStore, untrustedProject = true): AgentBackend {
...
export interface LocalService {
  store: Store; bridge: HierarchicalBridge; channel: LocalChannel;
...
export async function openService(c: Config, output: Writable, transport?: TransportAdapter, businessClock: () => number = Date.now): Promise<LocalService> {
  ...
  const controllerFactory = await ControllerFactory.open(c, event => {
    if (event.requestId) {
  ...
  await recoverHierarchyResources(c.stateRoot);
  const unlock = acquireLock(c.stateRoot);
  let bridge: HierarchicalBridge | undefined;
  ...
  try {
    ...
    const media = new MediaStore(c), channel = new LocalChannel(output);
    ...
    {
      invariant(c.orchestration, 'HIERARCHICAL_CONFIG_REQUIRED');
    ...
    process.stderr.write(JSON.stringify({ event: 'service.started', mode: 'hierarchical', schemaVersion: 4, transport: c.transport,
      bridgeModel: c.models![c.orchestration!.bridge.modelProfile], businessDefault: c.models![c.orchestration!.business.defaultModelProfile],
      routeInherits: 'bridge', recapModel: c.models![c.orchestration!.answers.recapModelProfile] }) + '\n');
