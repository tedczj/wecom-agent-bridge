    ...
    while (start < chunk.length) {
      ...
      if (bytes.length) {
        try {
          ...
          if (text.trim()) this.emit(record(JSON.parse(text)));
        } catch (e) { if (e instanceof BridgeError) throw e; throw new BridgeError('RPC_INVALID_JSON'); }
      }
      start = lf + 1;
    }
  }
...
export interface RpcOptions {
  command: string; args: string[]; cwd: string; env: NodeJS.ProcessEnv; maxFrameBytes: number;
  timeoutMs: number; killGraceMs: number; onSpawn?: (pid: number) => void; onStopped?: () => void;
}
interface Pending {
  command: string; resolve: (v: Record<string, unknown>) => void; reject: (e: Error) => void;
...
export class RpcProcess {
  ...
  constructor(private options: RpcOptions) {
    ...
    this.child.stdout.on('data', (chunk: Buffer) => {
      ...
      try { framer.push(chunk); } catch (e) { this.fail(e instanceof BridgeError ? e : new BridgeError('RPC_PROTOCOL')); }
    });
    this.child.stdout.on('end', () => { if (!this.stopping) {
      try { framer.end(); } catch { this.fail(new BridgeError('RPC_TRUNCATED_FRAME')); }
    } });
    ...
    this.child.on('exit', () => { if (!this.stopping) this.fail(new BridgeError('RPC_EXIT')); });
    if (this.child.pid) {
      try { options.onSpawn?.(this.child.pid); } catch { this.fail(new BridgeError('PROCESS_RECORD_FAILED')); }
    }
  }
  ...
  subscribe(onEvent: (e: Record<string, unknown>) => void, onError: (e: BridgeError) => void): () => void {
    this.listeners.add(onEvent); this.errors.add(onError);
    if (this.failure) queueMicrotask(() => { if (this.errors.has(onError)) onError(this.failure!); });
  ...
  private receive(frame: Record<string, unknown>): void {
    ...
    if (frame.type === 'response') {
      ...
      if (frame.success === true) p.resolve(frame);
      else if (frame.success === false) p.reject(new BridgeError('RPC_REJECTED'));
      else { p.reject(new BridgeError('RPC_PROTOCOL')); throw new BridgeError('RPC_PROTOCOL'); }
    } else for (const fn of this.listeners) fn(frame);
  }
  write(frame: Record<string, unknown>): Promise<void> {
    if (this.failure) return Promise.reject(this.failure);
    if (this.stopping) return Promise.reject(new BridgeError('RPC_STOPPED'));
    return new Promise((resolve, reject) => {
      this.child.stdin.write(JSON.stringify(frame) + '\n', error => {
        if (error) { const e = new BridgeError('RPC_STDIN_ERROR'); this.fail(e); reject(e); } else resolve();
      });
    });
  }
  ...
  async stop(): Promise<void> {
    if (this.stopping) return;
    this.stopping = true; this.fail(new BridgeError('RPC_STOPPED'));
    ...
    const signal = (s: NodeJS.Signals) => {
      if (pid) try { process.kill(-pid, s); } catch (e) {
