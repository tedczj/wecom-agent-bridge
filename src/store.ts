...
CREATE TABLE IF NOT EXISTS jobs (
 seq INTEGER PRIMARY KEY AUTOINCREMENT, task_id TEXT NOT NULL UNIQUE, bot_id TEXT NOT NULL, message_id TEXT NOT NULL,
 kind TEXT NOT NULL CHECK(kind IN ('agent','command')), session_key TEXT NOT NULL REFERENCES sessions(session_key),
...
export class Store {
    ...
    constructor(file: string, private c: Config, readOnly = false) {
        if (file !== ':memory:')
            for (const name of [file, file + '-wal', file + '-shm'])
                if (existsSync(name))
                    invariant(!lstatSync(name).isSymbolicLink(), 'UNSAFE_DB_PATH');
        ...
        invariant(version.user_version <= 1, 'SCHEMA_TOO_NEW');
        this.atomic(() => { this.db.exec(schema); this.db.exec('PRAGMA user_version=1;'); });
        if (file !== ':memory:')
            for (const name of [file, file + '-wal', file + '-shm'])
                if (existsSync(name))
    ...
    private createSession(base: string, actor: string, generation: number): Session {
        const key = base + ':' + generation;
        const now = Date.now();
        this.db.prepare('INSERT INTO sessions(session_key,base_key,generation,backend,workspace_id,actor_id,state,created_at,updated_at) VALUES (?,?,?,?,?,?,\'new\',?,?)').run(key, base, generation, this.c.backend, this.c.workspace.id, actor, now, now);
    ...
    reserve(incoming: Incoming, kind: 'agent' | 'command'): {
        job: Job;
        duplicate: boolean;
    } {
        return this.atomic(() => {
            const duplicate = this.db.prepare('SELECT * FROM jobs WHERE bot_id=? AND message_id=?').get(incoming.route.botId, incoming.messageId) as Job | undefined;
            if (duplicate)
                return { job: duplicate, duplicate: true };
            const base = baseKey(incoming.route, this.c.workspace.id, this.c.backend);
            const session = this.latest(base) ?? this.createSession(base, incoming.route.senderId, 0);
            if (kind === 'agent') {
                invariant(!this.blocked(), 'WORKSPACE_BLOCKED');
                const n = this.db.prepare("SELECT count(*) n FROM jobs WHERE kind='agent' AND status IN ('preparing','queued')").get() as {
                    n: number;
                };
                const own = this.db.prepare("SELECT count(*) n FROM jobs WHERE session_key=? AND kind='agent' AND status IN ('preparing','queued')").get(session.session_key) as {
                    n: number;
                };
                invariant(n.n < this.c.queue.maxPendingGlobal && own.n < this.c.queue.maxPendingPerSession, 'QUEUE_FULL');
            }
            const input: NormalizedInput = { taskId: randomUUID(), messageId: incoming.messageId, route: incoming.route, receivedAt: incoming.receivedAt, text: incoming.text, images: [], workspaceId: this.c.workspace.id, sessionKey: session.session_key, generation: session.generation };
            this.db.prepare('INSERT INTO jobs(task_id,bot_id,message_id,kind,session_key,route_json,input_json,status,created_at) VALUES (?,?,?,?,?,?,?,?,?)').run(input.taskId, incoming.route.botId, incoming.messageId, kind, session.session_key, JSON.stringify(incoming.route), JSON.stringify(input), kind === 'agent' ? 'preparing' : 'queued', Date.now());
            return { job: this.get(input.taskId), duplicate: false };
        });
    }
    prepared(taskId: string, images: ImageRef[]): boolean {
        const job = this.get(taskId);
        const input: NormalizedInput = JSON.parse(job.input_json);
        input.images = images;
    ...
    claim(): Job | undefined {
        return this.atomic(() => {
            if (this.blocked() || this.db.prepare("SELECT 1 FROM jobs WHERE status IN ('running','cancel_requested') LIMIT 1").get())
                return;
            // A later ready message cannot overtake an earlier preparing message in its session.
            const next = this.db.prepare("SELECT j.* FROM jobs j WHERE j.kind='agent' AND j.status='queued' AND NOT EXISTS (SELECT 1 FROM jobs p WHERE p.session_key=j.session_key AND p.seq<j.seq AND p.status IN ('preparing','queued','running','cancel_requested')) ORDER BY j.seq LIMIT 1").get() as Job | undefined;
            if (!next)
                return;
            this.db.prepare("UPDATE jobs SET status='running',started_at=? WHERE task_id=? AND status='queued'").run(Date.now(), next.task_id);
            return this.get(next.task_id);
        });
    }
    ...
    complete(taskId: string, status: JobStatus, text: string, code?: string, expected: JobStatus[] = ['running', 'cancel_requested', 'preparing', 'queued'], directParts?: string[]): boolean {
        ...
        return this.atomic(() => {
            ...
            if (!expected.includes(job.status))
                return false;
            if (job.status === 'cancel_requested' && status === 'succeeded') {
                status = 'cancelled';
                text = '任务已取消；可能已经产生部分代码修改，请检查工作目录。';
            }
            ...
            if (status === 'interrupted')
                this.db.prepare("UPDATE sessions SET state='tainted' WHERE session_key=?").run(job.session_key);
            if (directParts)
                invariant(job.kind === 'command' && directParts.every(p => Buffer.byteLength(p) <= this.c.reply.chunkBytes), 'REPLY_TOO_LARGE');
    ...
    owned(owner: Job, prefix: string): Job {
        ...
        const jobs = this.db.prepare('SELECT j.* FROM jobs j JOIN sessions s ON s.session_key=j.session_key WHERE s.base_key=? AND j.task_id LIKE ? AND j.kind=\'agent\' LIMIT 2').all(this.session(owner.session_key).base_key, prefix + '%') as unknown as Job[];
        invariant(jobs.length === 1, 'TASK_NOT_FOUND');
        return jobs[0]!;
    }
    cancel(taskId: string): JobStatus {
        return this.atomic(() => {
            ...
            if (['queued', 'preparing'].includes(job.status))
                this.complete(taskId, 'cancelled', '任务已取消，尚未开始执行。');
            else if (job.status === 'running')
                this.db.prepare("UPDATE jobs SET status='cancel_requested' WHERE task_id=? AND status='running'").run(taskId);
            return this.get(taskId).status;
        });
    }
    activeFor(owner: Job): Job | undefined {
        return this.db.prepare("SELECT j.* FROM jobs j JOIN sessions s ON s.session_key=j.session_key WHERE s.base_key=? AND j.kind='agent' AND j.status IN ('preparing','queued','running','cancel_requested') ORDER BY CASE WHEN j.status IN ('running','cancel_requested') THEN 0 ELSE 1 END,j.seq DESC LIMIT 1").get(this.session(owner.session_key).base_key) as Job | undefined;
    }
    newGeneration(owner: Job): number {
        invariant(!this.activeFor(owner), 'SESSION_BUSY');
        invariant(!this.blocked(), 'WORKSPACE_BLOCKED');
        const old = this.session(owner.session_key);
        const route = JSON.parse(owner.route_json);
    ...
    recover(): void {
        this.atomic(() => {
            for (const row of this.db.prepare("SELECT * FROM jobs WHERE status IN ('preparing','running','cancel_requested') OR (kind='command' AND status='queued')").all() as unknown as Job[]) {
                ...
                this.complete(row.task_id, interrupted ? 'interrupted' : 'failed', interrupted ? '执行被中断，可能已修改代码；请在本地检查进程和 git diff 后恢复。' : '输入准备或控制命令被中断，请重新发送。', interrupted ? 'EXECUTION_INTERRUPTED' : 'MEDIA_PREPARATION_INTERRUPTED');
            }
    ...
    activeMedia(): Set<string> { return new Set((this.db.prepare("SELECT task_id FROM jobs WHERE status IN ('preparing','queued','running','cancel_requested')").all() as Array<{
        task_id: string;
    }>).map(x => x.task_id)); }
    summary(owner?: Job): unknown {
        const where = owner ? ' WHERE s.base_key=?' : '';
        const args = owner ? [this.session(owner.session_key).base_key] : [];
