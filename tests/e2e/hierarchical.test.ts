...
import { readFileSync, mkdirSync, writeFileSync, appendFileSync, mkdtempSync, rmSync, realpathSync } from 'node:fs';
...
async function harness(t: TestContext, directoryModel?: Partial<ReturnType<typeof setup>['c']['codex']>,
  beforeStart?: (state: { store: Store; c: ReturnType<typeof parseConfig>; artifacts: ArtifactStore }) => Promise<void>) {
  ...
  const c = parseConfig({ ...f.c, codex: { ...f.c.codex, model: 'gpt-6-sol', reasoning: 'medium' },
      ...
      if (!text.startsWith('Initialize this management')) {
        ...
        if (this.role === 'bridge') {
          ...
          if (text.startsWith('search ')) {
            ...
            const result = await handler('route_delegate', { directoryRef: selected, intentKind: text.includes('history') ? 'history_query' : text.includes('switch') ? 'switch' : 'work' }, 'delegate'); parents.push(result);
  ...
  await beforeStart?.({ store, c, artifacts });
  ...
  return { ...f, c, second, store, bridge, artifacts, channel, controllerInputs, parents, searches, calls, refs, settle, managementModels,
...
test('OFFLINE hierarchical chain: request model override is temporary; explicit session preference persists only on that session', async t => {
  ...
  assert.deepEqual(h.calls.map(call => call.routing!.execution!.reasoning), ['medium', 'high', 'medium', 'high', 'high']);
  ...
  assert.ok(h.managementModels.every(model => model === 'gpt-6-sol/medium'));
});
