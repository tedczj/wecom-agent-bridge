...
let model={provider:'test-provider',id:process.env.FAKE_MODEL ?? 'gpt-5.6-terra',contextWindow:Number(process.env.FAKE_WINDOW ?? 1000000),input:mode==='no-images'?['text']:['text','image']},thinkingLevel='medium';
...
readline.createInterface({input:process.stdin}).on('line',line => {
  ...
  if(q.type === 'get_state') return reply(q,{model,thinkingLevel,isStreaming:streaming,isCompacting:false,pendingMessageCount:0,sessionId:id,sessionFile:file});
  if(q.type === 'get_available_models')return reply(q,{models:[model]});
  if(q.type === 'set_model') {model={...model,provider:q.provider,id:q.modelId};return reply(q,model);}
  if(q.type === 'set_thinking_level') {thinkingLevel=mode==='clamp-thinking'?'low':q.level;return reply(q);}
  ...
  capture({prompt:q.message,images,model,thinkingLevel,env:process.env}); history.push(q.message);writeFileSync(file,JSON.stringify({id,history}));streaming=true;reply(q);
